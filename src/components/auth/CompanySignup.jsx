...
//       else if (typeof v !== "undefined") formData.append(k, v);
//     });
...
//               }}
//               placeholder={certificateSelections.length === 0 ? "" : ""}
...
//             <div>
//               <label className="block text-sm font-medium text-black mb-2">Incorporation Certificate</label>
...
//         <button type="submit" className="w-full cursor-pointer bg-black text-white py-2.5 rounded-md font-semibold text-sm hover:opacity-90 transition">
//           Sign Up
//         </button>
//       </form>
//     </div>
...
export default function CompanySignup() {
  ...
  const onSubmit = async (data) => {
    ...
    // If publishing requirements, require license and file
    const pub = data.publishRequirements === "true" || data.publishRequirements === true;
    if (pub) {
      const hasLicense = data.licenseNumber && String(data.licenseNumber).trim().length >= 16;
      const hasFile = !!fileUploads.incorporationCertificate;
    ...
    // Build FormData and avoid appending empty websiteUrl
    const formData = new FormData();
    Object.entries(data).forEach(([k, v]) => {
      if (typeof v === "boolean") formData.append(k, v.toString());
    ...
    // canonical role string expected by server
    formData.append("role", "inspection_company");
    formData.append("certificates", JSON.stringify(certificateSelections));
    ...
    try {
      ...
      const json = await res.json().catch(() => ({}));
      if (!res.ok || !json.success) {
        // prefer server validation message if available
