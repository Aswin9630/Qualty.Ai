...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!result.success) {
        setError(result.errors?.[0]?.msg || result.message || "Signin failed");
      } else {
        dispatch(addUser(result.user));
        setError("");
        toast.success("Login successful!");
        setTimeout(() => {
          if (location.state?.redirectTo) {
            navigate(location.state.redirectTo, { state: location.state.payload });
          } else {
            navigate(`/${result?.user.role}/dashboard`);
          }
        }, 500);
    ...
    <div className="min-h-screen bg-white flex items-center justify-center">
      <div className="flex flex-col lg:flex-row items-center justify-center px-4 py-35 sm:py-25 lg:py-25 gap-5 w-full max-w-7xl mb-5">
        <div className="w-full lg:w-1/2 px-4 sm:px-6 lg:px-8 space-y-6 mb-8 lg:mb-0">
          ...
          <p className="text-gray-600 text-sm italic">“Quality is not just a checkbox — it’s a commitment to excellence.”</p>
          <div className="mt-4">
            <span className="inline-block bg-black text-white text-xs font-medium px-3 py-1 rounded-full shadow">Global. Verified. Transparent.</span>
          </div>
        </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      value={formData.password}
                      onChange={handleChange}
                      className="w-full pl-10 pr-12 py-3 bg-white border border-gray-300 text-black rounded-lg focus:outline-none focus:ring-2 focus:ring-black focus:border-transparent transition-all duration-200 disabled:opacity-50"
                    />
                    <button
                      type="button"
                      onClick={() => setShowPassword((s) => !s)}
                      className="absolute right-3 top-4 text-gray-600 items-center"
