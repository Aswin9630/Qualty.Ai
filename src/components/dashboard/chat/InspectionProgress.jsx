...
//                       ${isCompleted ? "bg-slate-900 text-white" : isCurrent && isInProgress ? "border-2 border-slate-900 text-slate-900 bg-white" : "border-2 border-slate-200 text-slate-400 bg-white"}`}
//                   >
//                     {isCompleted ? (
//                       <svg viewBox="0 0 16 16" className="w-4 h-4" fill="currentColor" xmlns="http://www.w3.org/2000/svg" aria-hidden>
...
//                     <div className={`text-sm sm:text-base font-semibold ${isCompleted || isCurrent ? "text-slate-900" : "text-slate-400"}`}>
//                       {label}
//                     </div>

//                     <div className={`inline-block text-xs sm:text-sm rounded-full px-2 py-0.5 mt-1 ${badgeClass}`}>
//                       {badgeLabel}
//                     </div>

//                     {stage.rejectionReason && <div className="text-xs text-red-600 mt-1">Reason: {stage.rejectionReason}</div>}

//                     <div className="mt-3">
//                       {isInspector && isCurrent && !isCompleted && (
//                         <button
//                           onClick={() => onAdvanceStage && onAdvanceStage(index)}
//                           className="px-3 py-1 text-xs sm:text-sm rounded-md border border-black text-black hover:bg-gray-100 transition"
//                         >
...
//                               className="px-3 py-1 text-xs sm:text-sm rounded-md bg-green-600 text-white hover:bg-green-700 transition"
//                             >
//                               Accept
//                             </button>
//                             <button
...
//                           <div className="text-xs sm:text-sm text-yellow-800 bg-yellow-50 px-2 py-1 rounded">
...
//                   <div className="mx-12 sm:mx-14 h-px bg-gray-200" />
//                 )}
//               </div>
//             );
//           })}
...
export default function InspectionProgress({
  ...
  // treat inspector and company the same
  const isInspectorLike = isInspector || isCompany;

  return (
    <aside className="w-full">
      <div className="bg-white p-3 sm:p-6 rounded-xl shadow-sm">
        <h2 className="text-sm sm:text-base font-semibold mb-4 text-center">
          Inspection Progress
        </h2>
          ...
          {stages.map((label, index) => {
            ...
            const badgeLabel = (() => {
              ...
              if (isPendingCustomer)
                return isInspectorLike ? "Pending Customer" : "In Progress";
              if (isRejected) return "Rejected";
              if (isInProgress) return "In Progress";
              return "Pending";
            })();
              ...
              <div key={index} className="relative">
                <div className="flex items-start py-4 px-2">
                  ...
                  <div className="flex-1 min-w-0 ml-3 sm:ml-4">
                    ...
                    <div
                      className={`inline-block text-xs sm:text-sm rounded-full px-2 py-0.5 mt-1 ${badgeClass}`}
                    >
                      {badgeLabel}
                    </div>
                    ...
                    {stage.rejectionReason && (
                      <div className="text-xs text-red-600 mt-1">
                        Reason: {stage.rejectionReason}
                      </div>
                    )}

                    <div className="mt-3">
                      {isInspectorLike && isCurrent && !isCompleted && (
                        <button
                          onClick={() =>
                            onAdvanceStage && onAdvanceStage(index)
                          }
                          className="px-3 py-1 text-xs sm:text-sm rounded-md border border-black text-black hover:bg-gray-100 transition"
                        >
                          {index === stages.length - 1
                      ...
                      {!isInspectorLike && isPendingCustomer && isCurrent && (
                        <div className="flex flex-col gap-2">
                          <div className="flex gap-2">
                            <button
                              onClick={() =>
                                onAcceptStage && onAcceptStage(index)
                              }
                              className="px-3 py-1 text-xs sm:text-sm rounded-md bg-green-600 text-white hover:bg-green-700 transition"
                            >
                              Accept
                          ...
                          <div className="text-xs sm:text-sm text-yellow-800 bg-yellow-50 px-2 py-1 rounded">
                            Waiting for your approval — Accept or Reject to
                            proceed
                          </div>
                        </div>
                      )}

                      {isRejected && isInspectorLike && (
                        <div className="text-xs text-red-600 mt-1">
                          Rejected by customer — revise and Finish again
                        </div>
                      )}

                      {isRejected && !isInspectorLike && (
                        <div className="text-xs text-red-600 mt-1">
                          Rejected — please discuss with inspector
                        </div>
                      )}
                    </div>
                  </div>
