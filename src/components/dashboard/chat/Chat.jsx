...
/**
 * Chat component with:
 * - socket join / message send
 * - file upload (image + other files)
 * - thumbnails and full-screen image modal preview on click
 * - scroll-to-bottom on new messages
 * - progress updates via socket
 *
 * Place this file at the same path where you import it.
 */
export default function Chat() {
  const { targetId, orderId } = useParams();
  const [messages, setMessages] = useState([]);
          ...
          >
            <div className="relative max-w-[95%] max-h-[95%]">
              ...
              <img
                src={modalImage}
                alt="Preview"
                className="max-w-full max-h-full rounded shadow-lg mx-auto"
              />
            </div>
          </div>
        )}
