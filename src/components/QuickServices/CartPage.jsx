          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              {items.map((it) => (
                <div key={it._id} className="bg-white p-4 rounded-lg shadow-sm flex flex-col sm:flex-row justify-between items-start gap-4">
                  ...
                  <div className="flex flex-row sm:flex-col items-end gap-3 w-full sm:w-auto justify-between sm:justify-start">
                    <div className="text-base sm:text-lg font-semibold text-gray-900">{it.isEnquiry ? "-" : (it.currency === "INR" ? "₹" : "$") + it.price}</div>
            ...
            <aside className="bg-white p-6 rounded-lg shadow-md h-fit sticky top-20 order-last lg:order-none">
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="font-semibold text-gray-900">{totalsLoading
      ? (clientCurrency === "INR" ? "₹" : "$") + clientSubtotal
      : (serverTotals.currency === "INR" ? "₹" : "$") + serverTotals.subtotal}</div>
              </div>

              <div className="flex justify-between items-center mb-2">
                <div className="text-sm text-gray-600">Tax</div>
                <div className="text-sm text-gray-600">{totalsLoading
      ? "-"
      : (serverTotals.currency === "INR" ? "₹" : "$") + serverTotals.tax}</div>
              </div>
              ...
              <div className="flex justify-between items-center text-lg font-bold mb-4">
                ...
                <div>{totalsLoading
      ? (clientCurrency === "INR" ? "₹" : "$") + clientSubtotal
      : (serverTotals.currency === "INR" ? "₹" : "$") + serverTotals.total}</div>
              </div>
