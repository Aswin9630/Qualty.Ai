...
import ConfirmModal from "../ConfirmModal";
import { useCart } from "../../hooks/useCart";
import { BASE_URL,formatMajor } from "../../utils/constants";
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="lg:col-span-2 space-y-4">
              {items.map((it) => (
                <div key={it._id} className="bg-white p-4 rounded-lg shadow-sm flex flex-col sm:flex-row justify-between items-start gap-4">
                  ...
                  <div className="flex flex-row sm:flex-col items-end gap-3 w-full sm:w-auto justify-between sm:justify-start">
                    <div className="text-base sm:text-lg font-semibold text-gray-900">{it.isEnquiry ? "-" : formatMajor(it.price, it.currency)}</div>
                    <button onClick={() => onRemoveClick(it._id)} className="text-sm cursor-pointer text-red-500 hover:underline">Remove</button>
                  </div>
                </div>
              ))}
            ...
            <aside className="bg-white p-6 rounded-lg shadow-md h-fit sticky top-20 order-last lg:order-none">
              ...
              <div className="flex justify-between items-center mb-2">
                <div className="text-sm text-gray-600">Subtotal</div>
                <div className="font-semibold text-gray-900">{totalsLoading ? formatMajor(clientSubtotal, clientCurrency) : formatMajor(serverTotals.subtotal, serverTotals.currency)}</div>
              </div>

              <div className="flex justify-between items-center mb-2">
                <div className="text-sm text-gray-600">Tax</div>
                <div className="text-sm text-gray-600">{totalsLoading ? "-" : formatMajor(serverTotals.tax, serverTotals.currency)}</div>
              </div>
              ...
              <div className="flex justify-between items-center text-lg font-bold mb-4">
                <div>Total</div>
                <div>{totalsLoading ? formatMajor(clientSubtotal, clientCurrency) : formatMajor(serverTotals.total, serverTotals.currency)}</div>
              </div>
