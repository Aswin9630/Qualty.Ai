...
import { Download } from "lucide-react";
import { BASE_URL } from "../../utils/constants";
import { generatePdfFromOrderHtml } from "./invoicePdfServiceHtml";
import { formatMajor } from "../../utils/constants";
                    ...
                    <div className="flex-1">
                      ...
                      <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
                          ...
                          <ul className="space-y-3">
                            {order.items?.map((it, idx) => (
                              <li key={idx} className="flex items-start justify-between gap-4">
                                ...
                                <div className="text-sm sm:text-base font-semibold whitespace-nowrap">{formatMajor(it.price, it.currency || order.currency)}</div>
                              </li>
                            ))}
                          </ul>
                        </div>
                        ...
                        <div>
                          <div className="text-xs sm:text-sm text-gray-600 mb-2">Payment</div>
                          <div className="text-xs sm:text-sm text-gray-700 space-y-2">
                            <div className="text-sm sm:text-base"><strong>Subtotal:</strong> {formatMajor(order.subtotal, order.currency)}</div>
                            <div className="text-sm sm:text-base"><strong>Tax:</strong> {formatMajor(order.tax, order.currency)}</div>
                            <div className="mt-2 text-base sm:text-lg font-semibold">{formatMajor(order.total, order.currency)}</div>
                            <div className="mt-3 text-xs sm:text-sm text-gray-500">Payment ID: {order.paymentId || order.razorpay_order_id || "—"}</div>
                          </div>
              ...
              <div className="bg-white rounded-lg p-4 shadow-sm">
                ...
                <ul className="space-y-2 text-xs sm:text-sm">
                  {orders.slice(0, 5).map((o) => (
                    <li key={o._id} className="flex items-center justify-between">
                      <div className="text-xxs sm:text-xs">{formatDateTime(o.createdAt)}</div>
                      <div className="font-medium text-sm sm:text-base">{formatMajor(o.total, o.currency)}</div>
                    </li>
                  ))}
                </ul>
              </div>
          ...
          <div className="w-full md:w-3/5 lg:w-2/5 bg-white h-full overflow-auto p-6">
            ...
            <div className="mt-4 space-y-4">
                ...
                <ul className="space-y-3">
                  {selectedOrder.items?.map((it, idx) => (
                    <li key={idx} className="flex items-start justify-between gap-4">
                      ...
                      <div className="text-sm sm:text-base font-semibold whitespace-nowrap">{formatMajor(it.price, it.currency || selectedOrder.currency)}</div>
                    </li>
                  ))}
                </ul>
              </div>
              ...
              <div className="bg-white p-4 rounded shadow-sm">
                <div className="text-xs sm:text-sm text-gray-600 mb-2">Payment summary</div>
                <div className="text-sm sm:text-sm text-gray-700 space-y-2">
                  <div className="flex justify-between text-xs sm:text-sm"><div>Subtotal</div><div>{formatMajor(selectedOrder.subtotal, selectedOrder.currency)}</div></div>
                  <div className="flex justify-between text-xs sm:text-sm"><div>Tax</div><div>{formatMajor(selectedOrder.tax, selectedOrder.currency)}</div></div>
                  <div className="flex justify-between font-bold text-sm sm:text-base"><div>Total</div><div>{formatMajor(selectedOrder.total, selectedOrder.currency)}</div></div>
                  <div className="mt-3 text-xs sm:text-sm text-gray-500">Payment ID: {selectedOrder.paymentId || selectedOrder.razorpay_order_id}</div>
                </div>
              </div>
